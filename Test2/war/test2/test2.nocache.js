...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "test2" because\n';
 ...
 * "/test2.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['test2'].superdevmode = true;

  var expectedSuffix = '/test2.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + test2 + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:test2';
  var devModeSessionKey = '__gwtDevModeSession:test2';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/test2/test2.nocache.js';
  ...
  $wnd.__gwt_activeModules['test2'].canRedirect = true;
  $wnd.__gwt_activeModules['test2'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/test2?user.agent=' + ua + '&_callback=' + callback;
